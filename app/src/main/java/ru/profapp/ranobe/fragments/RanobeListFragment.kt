...
class RanobeListFragment : Fragment() {
    ...
    private fun getRulateWebFavorite(): Single<List<Ranobe>> {
        ...
        if (token.isBlank()) return Single.just(listOf())
    ...
    override fun onCreateView(
            ...
            iBrLFragmentSync.setOnClickListener {
                ...
                builder.setTitle(getString(R.string.load_web_bookmarks))
                    ...
                    .setNegativeButton("Cancel") { dialog, _ ->
                        ...
                        Single.zip(getRulateWebFavorite().onErrorReturn {
                            listOf(Ranobe(Error))
                        },
                            ...
                            io.reactivex.functions.Function4<List<Ranobe>, List<Ranobe>, List<Ranobe>, List<Ranobe>, List<Ranobe>> { Rulate, RanobeRf, RanobeHub, local ->
            ...
            tVSortOrder.setOnClickListener {
                ...
                builder.setTitle(getString(R.string.load_web_bookmarks))
                    ...
                    .setSingleChoiceItems(stringItems, currentItem) { dialog, which ->
                        ...
                        dialog.dismiss()
                    }.setCancelable(true).setNegativeButton("Cancel") { dialog, _ ->
                        dialog.cancel()
                    }
        ...
        if (fragmentType != Constants.FragmentType.Favorite && fragmentType != Constants.FragmentType.Search && fragmentType != Constants.FragmentType.Saved) {
        ...
        request = loader
                ...
                if (fragmentType == Constants.FragmentType.Favorite) {
                    for (ranobe in ranobes) {
                        ...
                        if (chapterList.isNotEmpty()) {
                            ...
                            if (!wasRead) {
                                ranobe.newChapters = chapterlist2.size
                            }
            ...
            .subscribe({ result ->
                ...
                if (fragmentType == Constants.FragmentType.Saved) {
                    ...
                    Snackbar.make(swipeRefreshLayout, R.string.saved_info, Snackbar.LENGTH_SHORT)
                        .show()
                }
            ...
            if (error is UnknownHostException) {
                ...
                for (err in error.exceptions) {
                    if (err is UnknownHostException) {
                        Toast.makeText(mContext, R.string.error_connection, Toast.LENGTH_SHORT)
                            .show()
